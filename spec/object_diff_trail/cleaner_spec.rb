  ...
  ::RSpec.describe Cleaner, versioning: true do
    describe "clean_versions!" do
      ...
      it "baseline test setup" do
        expect(ObjectDiffTrail::Version.count).to(eq(9))
        animals.each { |animal| expect(animal.versions.size).to(eq(3)) }
      end

      context "no options provided" do
        it "removes extra versions for each item" do
          ObjectDiffTrail.clean_versions!
          expect(ObjectDiffTrail::Version.count).to(eq(3))
      ...
      context "keeping 2" do
        it "keeps two records, instead of the usual one" do
          ObjectDiffTrail.clean_versions!(keeping: 2)
          expect(ObjectDiffTrail::Version.all.count).to(eq(6))
      ...
      context "with the :date option" do
        it "only deletes versions created on the given date" do
          ...
          animal.update_attribute(:name, FFaker::Name.name)
          expect(ObjectDiffTrail::Version.count).to(eq(10))
          expect(animal.versions.size).to(eq(4))
          expect(animal.object_diff_trail.versions_between(date, (date + 1.day)).size).to(eq(3))
          ObjectDiffTrail.clean_versions!(date: date)
          expect(ObjectDiffTrail::Version.count).to(eq(8))
      ...
      context "with the :item_id option" do
        context "single ID received" do
          it "only deletes the versions for the Item with that ID" do
            ObjectDiffTrail.clean_versions!(item_id: animal.id)
            expect(animal.versions.size).to(eq(1))
            expect(ObjectDiffTrail::Version.count).to(eq(7))
          end
        end

        context "collection of IDs received" do
          it "only deletes versions for the Item(s) with those IDs" do
            ObjectDiffTrail.clean_versions!(item_id: [animal.id, dog.id])
            expect(animal.versions.size).to(eq(1))
            expect(dog.versions.size).to(eq(1))
            expect(ObjectDiffTrail::Version.count).to(eq(5))
          end
        end
      end
      ...
      context "options combinations" do
        context ":date" do
          ...
          it "baseline test setup" do
            date = animal.versions.first.created_at.to_date
            expect(ObjectDiffTrail::Version.count).to(eq(11))
            [animal, dog].each do |animal|
              expect(animal.versions.size).to(eq(4))
          ...
          context "and :keeping" do
            it "restrict cleaning properly" do
              date = animal.versions.first.created_at.to_date
              ObjectDiffTrail.clean_versions!(date: date, keeping: 2)
              [animal, dog].each do |animal|
          ...
          context "and :item_id" do
            it "restrict cleaning properly" do
              date = animal.versions.first.created_at.to_date
              ObjectDiffTrail.clean_versions!(date: date, item_id: dog.id)
              dog.versions.reload
          ...
          context ", :item_id, and :keeping" do
            it "restrict cleaning properly" do
              date = animal.versions.first.created_at.to_date
              ObjectDiffTrail.clean_versions!(date: date, item_id: dog.id, keeping: 2)
              dog.versions.reload
        ...
        context ":keeping and :item_id" do
          it "restrict cleaning properly" do
            ObjectDiffTrail.clean_versions!(keeping: 2, item_id: animal.id)
